...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ob, reward, done, info = env.step_20(data)
...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            remote.send((fr))
        elif cmd == 'get_current_station':
            remote.send((env.current_station))
        else:
            raise NotImplementedError
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def reset(self):
        ...
        return np.stack(obs), np.stack(share_obs), np.stack(available_actions)  # shape: (n_thread, n_agents, obs_dim)

    def get_current_station(self):
        for remote in self.remotes:
            remote.send(('get_current_station', None))
        return np.stack([remote.recv() for remote in self.remotes])
...
def chooseworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        cmd, data = remote.recv()
        if cmd == 'step':
            ob, s_ob, reward, done, info, available_actions = env.step_20(data)
            remote.send((ob, s_ob, reward, done, info, available_actions))
        elif cmd == 'reset':
...
class DummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [env.step_20(a) for (a, env) in zip(self.actions, self.envs)]
